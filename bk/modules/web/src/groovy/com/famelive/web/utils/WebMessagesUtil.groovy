...
import com.famelive.web.constants.WebConstants
import org.codehaus.groovy.grails.commons.DefaultGrailsApplication

class WebMessagesUtil {
    ...
    private File[] getPropertiesFiles() {
        File messagesDIR = grailsApplication.mainContext.getResource(WebConstants.Messages_PATH).getFile()
    ...
    private void updateMessageProperties(Map commonPropertyMap, Map webPropertyMap) {
        messageSource = new Properties()
        Map propertyMap = [:]
        propertyMap.putAll(commonPropertyMap)
        propertyMap.putAll(webPropertyMap)
        messageSource.putAll(propertyMap)
    }

    private void updateMessageProperties() {
        Map webPropertyMap = getWebPropertyMap()
        Map commonPropertyMap = getCommonPropertyMap()
        updateMessageProperties(commonPropertyMap, webPropertyMap)
    }
}

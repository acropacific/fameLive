 ...
 */
package com.wowza.wms.plugin.collection.multitrackvod;
...
import com.wowza.wms.plugin.collection.module.ModuleMultiTrackVOD;
			...
			if(!queryMap.isEmpty())
			{
	            for(int i = 0; i < ModuleMultiTrackVOD.PROPERTY_INDEXES.length; i++)
	            {
	                ...
	                if(queryMap.containsKey(indexStr))
	                {
	                    try
	                    {
	                        int index = Integer.parseInt((String)queryMap.get(indexStr));

...
public class OutputStream extends Thread
{
	...
	public OutputStream(IApplicationInstance appInstance, String outputName, long startTime, long sortDelay, boolean useOriginalTimecodes)
	{
		...
		this.debugLog = appInstance.getProperties().getPropertyBoolean("avMixDebugLog", this.debugLog);
					...
					if (packets.isEmpty() && videoSource == null && audioSource == null)
					{
						if (sortDelay >= 0)
						{
							if (useOriginalTimecodes)
							{
								...
								{
									if (debugLog)
										logger.info(CLASS_NAME + ".run(): " + "[" + appInstance.getContextStr() + "/" + outputName + " shutdown : " + packets.size() + " : " + (now - sortDelay) + " : " + (lastTC + delayOffset) + " : " + startTime + "]", WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
									running = false;
									break;
								}
							}
	...
	private void processVideoSource()
	{
		...
		if (stream != videoSource)
		{
			...
			if (pendingVideoSwitch && useOriginalTimecodes && waitForKeyframe)
			{
				...
				if(videoSwitchTimecode == -1)
				{
					...
					{
						if (debugLog)
							logger.info(CLASS_NAME + ".processVideoSource(): " + "[" + appInstance.getContextStr() + "/" + outputName + " pending switch waiting for new stream to catch up " + (newKey != null ? newKey.getAbsTimecode() : 0) + ": " + lastProcessedVideoTC + "]", WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
						return;
					}
				}
			}
		...
		int startIdx = (this.videoSeq == -1 ? 0 : (int)(this.videoSeq - startSeq + 1));
		if (startIdx < 0)
			startIdx = 0;
	...
	private void processAudioSource()
	{
		...
		int startIdx = (this.audioSeq == -1 ? 0 : (int)(this.audioSeq - startSeq + 1));
		if (startIdx < 0)
			startIdx = 0;
	...
	private void processOutput()
	{
		...
		while (true)
		{
			...
			if(useOriginalTimecodes)
			{
				...
				{
					if (debugLog)
						logger.info(CLASS_NAME + ".processOutput(): " + "[" + appInstance.getContextStr() + "/" + outputName + " no packets available. (packet.getAbsTimecode() + delayOffset > now - sortDelay) timecode: " + packet.getAbsTimecode() + ", delayOffset: " + delayOffset + ", now: " + now + ", sortDelay: " + sortDelay +"]", WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
					break;
				}
			}
			...
			{
				if (debugLog)
					logger.info(CLASS_NAME + ".processOutput(): " + "[" + appInstance.getContextStr() + "/" + outputName + " no packets available. (packet.getAbsTimecode() > now - sortDelay) timecode: " + packet.getAbsTimecode() + ", now: " + now + ", sortDelay: " + sortDelay + "]", WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
				break;
			}
	...
	private boolean initPublisher()
	{
		...
		if (publisher != null)
		{
			...
			this.flushInterval = stream.getProperties().getPropertyLong("flushInterval", appInstance.getProperties().getPropertyLong("avMixflushInterval", this.flushInterval));
	...
	{
		this.audioName = audioSourceName;
	}
	...
	{
		this.videoName = videoSourceName;
	}

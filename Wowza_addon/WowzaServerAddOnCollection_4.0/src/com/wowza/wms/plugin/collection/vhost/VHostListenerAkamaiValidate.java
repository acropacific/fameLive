package com.wowza.wms.plugin.collection.vhost;
		...
		private boolean doSignatureAuth(String req, String path)
		{
			...
			try
			{
				...
				case AUTH_SIGNATURE_VERSION_3:
					Key key = new SecretKeySpec(secret.getBytes(CHAR_ENCODE_STR), "HmacMD5");
					if (debugLog)
						logger.info(CLASS_NAME + ".doSignatureAuth building version " + version + "create key [sectet: " + DebugUtils.formatBytesShort(secret.getBytes(CHAR_ENCODE_STR)) + ", key: " + DebugUtils.formatBytesShort(key.getEncoded()) + "]", WMSLoggerIDs.CAT_vhost,

...
import com.wowza.wms.plugin.collection.module.ModuleRefererValidate;
import com.wowza.wms.plugin.collection.referervalidate.SessionInfo;
...
public class HttpRefererValidate extends HTTProvider2Base
{
	...
	public void onBind(IVHost vhost, HostPort hostPort)
	{
		...
		else
		{
			...
			lock = new Object();
			timer = new Timer();
			timer.schedule(new ExpireTimer(), 0, this.sessionCheckInterval);
		}
	...
	public void onHTTPRequest(IVHost vhost, IHTTPRequest req, IHTTPResponse resp)
	{
		...
		boolean allowBlankReferers = ModuleRefererValidate.ALLOW_BLANK_REFERERS;
		long sessionDuration = ModuleRefererValidate.SESSION_VALID_DURATION_DEFAULT; // 10 minutes
		...
		while (true)
		{
			...
			if (streamName != null)
				streamName = ModuleRefererValidate.decodeStreamName(streamName);
			...
			referers = props.getPropertyStr(ModuleRefererValidate.PROP_NAME_PREFIX + "Referers", referers);
			if (debugLog)
			...
			allowBlankReferers = props.getPropertyBoolean(ModuleRefererValidate.PROP_NAME_PREFIX + "AllowBlankReferers", allowBlankReferers);
			if (debugLog)
			...
			sessionDuration = props.getPropertyLong(ModuleRefererValidate.PROP_NAME_PREFIX + "SessionValidDuration", sessionDuration);

			addNewSession(appName, appInstName, streamName, ipAddress, queryStr, referer, sessionDuration);

			break;

		}

		boolean return404OnNotValidated = props != null ? props.getPropertyBoolean(ModuleRefererValidate.PROP_NAME_PREFIX + "Return404OnNotValidated", true) : true;
		if (!validRequest && return404OnNotValidated)
		{
			resp.setResponseCode(404);
			return;
		}
		imagePath = props.getPropertyStr(ModuleRefererValidate.PROP_NAME_PREFIX + "ImagePath", "");

		if (!StringUtils.isEmpty(imagePath))
		{
			imagePath = decodeImagePath(appInstance, streamName, validRequest, imagePath);

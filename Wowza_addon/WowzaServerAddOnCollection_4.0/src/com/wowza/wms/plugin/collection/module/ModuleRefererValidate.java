...
import com.wowza.wms.plugin.collection.http.HttpRefererValidate;
...
public class ModuleRefererValidate extends ModuleBase
{
	...
	private boolean checkSession(String ipAddress, String streamName)
	{
		...
		while (iter.hasNext())
		{
			...
			if (debugLog)
			{
				logger.info(MODULE_NAME + ".checkSession [session: " + session + "] [ passFlag: " + pass + "] ", WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
			}
			...
			if (pass && !streamName.equalsIgnoreCase(session.getStreamName()))
			{

...
grails -Dgrails.env=$DEPLOY_ENV war -DbuildModule=api --non-interactive ~/tomcat/apache-tomcat-6.0.41/webapps/ROOT.war
kill -9 `cat $CATALINA_PID`;
rm -rf ~/tomcat/apache-tomcat-6.0.41/webapps/ROOT
rm -rf $CATALINA_PID
cd
if [ -e $FILE_POST ];
then

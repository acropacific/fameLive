...
<xsl:stylesheet version="1.0" xmlns:codegen="urn:weborb-cogegen-xslt-lib:xslt" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="libs.folder.additional.content">
     <file path="frameworks/robotlegs-framework-v1.4.0.swc" hideContent="true" />
  </xsl:template>
    ...
    </xsl:call-template><![CDATA[package ]]><xsl:value-of select="$serviceNamespace"/><![CDATA[.views
{
    ...
    public class ]]><xsl:value-of select="$serviceName"/><![CDATA[Mediator extends Mediator
    {
      ...
      <xsl:if test="$foundMethod">
        ...
        private function on]]><xsl:value-of select="$methodNameSuffix"/><![CDATA[ResultChanged(event:]]><xsl:value-of select="$serviceName"/><![CDATA[ModelEvent):void
        {
            ...
            var result:String = ObjectUtil.toString( model.]]><xsl:value-of select="$eventStr"/><![CDATA[Result );
            if (result == "")
    ...
    <file name="{$serviceName}View.mxml"><![CDATA[<?xml version="1.0" encoding="utf-8"?>

...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.java.data.oracle.update">
    ...
    <xsl:if test="$editable">
      final String SqlUpdate = "Update \"<xsl:value-of select="$schema"/>\".\"<xsl:value-of select="$table" />\" Set "
    ...
    <xsl:for-each select="key('dependent',current()/xs:key/@name)[@wdm:Schema = current()/xs:key/@wdm:Schema]">
      ...
      <xsl:variable name="child-table-pk-schema" select="xs:key/@wdm:Schema" />
          ...
          {
            <xsl:value-of select="codegen:getServerNamespace(@wdm:Schema)"/>.<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>DataMapper dataMapper = new <xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>DataMapper(getDatabase());

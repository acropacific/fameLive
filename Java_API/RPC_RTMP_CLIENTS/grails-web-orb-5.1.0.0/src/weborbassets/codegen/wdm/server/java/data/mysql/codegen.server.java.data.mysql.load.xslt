...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.java.data.mysql.load">
    ...
    <xsl:for-each select="xs:complexType/xs:attribute">
      ...
      <xsl:choose>
        ...
        if( dataReader.getObject(<xsl:value-of select="position()" />) != null )        
          ...
          <xsl:value-of select="$functionParam" />.set<xsl:value-of select="$property"/>( new <xsl:value-of select="codegen:javaDataType(@type)" />( dataReader.get<xsl:value-of select="codegen:javaDataType(@type)" />( <xsl:value-of select="position()" /> ) ) );
            </xsl:when>   
            <xsl:otherwise>
          <xsl:value-of select="$functionParam" />.set<xsl:value-of select="$property"/>( (<xsl:value-of select="codegen:javaDataType(@type)" />)dataReader.getObject(<xsl:value-of select="position()" />) );
            </xsl:otherwise>
            ...
            while( result.next() )
            {
              ...
              if(result.getObject("<xsl:value-of select="@name"/>") != null)
                ...
                item.set<xsl:value-of select="$property"/>( ( <xsl:value-of select="codegen:javaDataType(@type)" />)result.getObject( "<xsl:value-of select="@name"/>" ) );
                    ...
                    <xsl:when test="codegen:isNumericType(@type) or @type = 'xs:decimal'">
    ...
    <xsl:for-each select="xs:keyref">
      ...
      protected ArrayList findBy<xsl:value-of select="$parent-property"/>(<xsl:value-of select="codegen:getClassName($parent-table,$parent-schema)"/> domainObject, QueryOptions queryOptions) throws Exception
      {
    ...
    <xsl:if test="key('dependent',current()/xs:key/@name)">
      protected void loadRelations(DomainObject domainObject, QueryOptions queryOptions) throws Exception
      {
        ...
        <xsl:for-each select="key('dependent',current()/xs:key/@name)">
          ...
          {
            <xsl:value-of select="codegen:getClassName($child-table,$child-schema)" />DataMapper dataMapper = new <xsl:value-of select="codegen:getClassName($child-table,$child-schema)" />DataMapper(getDatabase());

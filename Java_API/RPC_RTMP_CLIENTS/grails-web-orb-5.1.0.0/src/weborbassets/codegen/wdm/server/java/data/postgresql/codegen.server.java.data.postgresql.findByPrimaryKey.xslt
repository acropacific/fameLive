...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.java.data.postgresql.findByPrimaryKey">
    <xsl:variable name="table" select="@name"   />
    <xsl:variable name="schema" select="@wdm:Schema"   />
    <xsl:variable name="serverNS" select="codegen:getServerNamespace($schema)" />
    <xsl:variable name="class-name" select="codegen:getClassName($table,$schema)"/>
    <xsl:variable name="full-class-name" select="concat($serverNS, '.', $class-name)" />
    ...
    <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
      ...
      {
        <xsl:value-of select="$full-class-name" /><xsl:text> </xsl:text><xsl:value-of select="$functionParam" /> = (<xsl:value-of select="$full-class-name" />) arg;
        DatabaseConnectionMonitor monitor = null;
		...
		<xsl:choose>
		  ...
		  sqlCommand.setObject( <xsl:value-of select="position()"/>, new java.sql.Date( <xsl:value-of select="$functionParam" />.get<xsl:value-of select="codegen:getPropertyName($table,$schema,@name)" />().getTime() ) );
		    </xsl:when>
			<xsl:otherwise>
		  sqlCommand.setObject( <xsl:value-of select="position()"/>, <xsl:value-of select="$functionParam" />.get<xsl:value-of select="codegen:getPropertyName($table,$schema,@name)" />() );
			</xsl:otherwise>
		</xsl:choose>		
        </xsl:for-each>
		  ResultSet dataReader = sqlCommand.executeQuery();
    ...
    <xsl:if test="position() != 1"> + </xsl:if>" \"<xsl:value-of select="$table"/>\".\"<xsl:value-of select="@name" />\" = ? <xsl:if test="position() != last()"> and </xsl:if>"
      ...
      {
        <xsl:value-of select="$full-class-name"/> _<xsl:value-of select="$class-name"/> = (<xsl:value-of select="$full-class-name"/>)domainObject;
        PreparedStatement sqlCommand = getDatabase().createCommand( DataMapper.modifyQueryForCheckIn(sqlQuery,CheckInSql) );

      <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
        sqlCommand.setObject( <xsl:value-of select="position()"/>, _<xsl:value-of select="$class-name"/>.get<xsl:value-of select="codegen:getPropertyName($table,$schema,@name)" />() );
      </xsl:for-each>

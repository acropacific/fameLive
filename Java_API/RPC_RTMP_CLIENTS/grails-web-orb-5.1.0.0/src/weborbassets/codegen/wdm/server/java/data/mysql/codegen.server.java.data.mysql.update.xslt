...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.java.data.mysql.update">
    ...
    <xsl:if test="$editable">
      final String SqlUpdate = "Update `<xsl:value-of select="@name" />` Set "
      ...
      <xsl:if test="$editable">
        ...
        <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
          ...
          if(<xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property"/>() != null <xsl:if test="@type = 'xs:string'"><![CDATA[&&]]> !<xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property"/>().equals("")</xsl:if> <xsl:if test="codegen:IsBinary( @type)"><![CDATA[&&]]> <xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property"/>().length != 0 </xsl:if> )
              ...
              <xsl:choose>
                <xsl:when test="@type = 'xs:date'">
            sqlCommand.setObject(<xsl:value-of select="position() + $startPosition"/>, new java.sql.Timestamp( <xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property"/>().getTime() ) );
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:for-each select="key('dependent',current()/xs:key/@name)">
      <xsl:variable name="child-table-pk" select="xs:key/@name" />
          ...
          if(<xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property-name" />() != null)
          {
            ...
            dataMapper.save((<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>)<xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property-name" />());
          }
          ...
          <![CDATA[&&]]> <xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property-name" />Items().size() &gt; 0)
          {
            ...
            for( int i = 0; i &lt; <xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property-name" />Items().size(); i++ )
              dataMapper.save((<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>)<xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property-name" />Items().get(i));
          }
            </xsl:otherwise>
          </xsl:choose>
            </xsl:for-each>

...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.java.data.mssql.create">
    ...
    private final String SqlCreate = "Insert Into [<xsl:value-of select="@wdm:Schema" />].[<xsl:value-of select="@name" />] ( "
        ...
        <xsl:for-each select="key('table',$child-table-pk)[@wdm:Schema = $child-table-pk-schema]">
          ...
          <xsl:choose>
            <xsl:when test="$identity">
            	sqlCommand.execute();
                sqlCommand = getDatabase().createCommand( "select @@IDENTITY;");
                
	          if(sqlCommand.execute())
	          {
	          	SQLServerResultSet resultSet = (SQLServerResultSet)sqlCommand.getResultSet();
              resultSet.next();

...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.server.java.data.oracle.create">
    ...
    <xsl:variable name="timestamp" select="boolean(xs:complexType/xs:attribute[@type='xs:time'])" />
      ...
      private final String SqlCreate = "Insert Into \"<xsl:value-of select="$schema"/>\".\"<xsl:value-of select="$table" />\" ( "<xsl:for-each select="xs:complexType/xs:attribute[not(@msdata:AutoIncrement='true') and not(@msdata:ReadOnly='true')]">
     + " \"<xsl:value-of select="@name" />\"<xsl:if test="position() != last()">,</xsl:if>"
     </xsl:for-each>+ " ) Values ( "<xsl:for-each select="xs:complexType/xs:attribute[not(@msdata:AutoIncrement='true') and not(@msdata:ReadOnly='true')]">+ " ?<xsl:if test="position() != last()">,</xsl:if> "</xsl:for-each>+ " ) ";
      ...
      public <xsl:value-of select="$class-name" /> createTyped( <xsl:value-of select="$class-name" /><xsl:text> </xsl:text><xsl:value-of select="$functionParam" /> ) throws Exception 
      {
        ...
        try
        {
          ...
          ResultSet result = getDatabase().createCommand(                 
          "select ud.REFERENCED_NAME from USER_DEPENDENCIES ud " +
            "where ud.NAME=(select utc.TRIGGER_NAME " +
            "from USER_TRIGGER_COLS utc " +
            "where utc.TABLE_NAME='<xsl:value-of select="$table" />' and utc.COLUMN_NAME='<xsl:value-of select="xs:complexType/xs:attribute[@msdata:AutoIncrement='true']/@name"/>') and ud.REFERENCED_TYPE='SEQUENCE' and ud.TYPE='TRIGGER'"
          ).executeQuery();
          result.next();
          String identity_query = "select \"" + result.getString( 1 ) + "\".currval from dual";
          result.close();
          </xsl:if>           
        ...
        <xsl:for-each select="xs:complexType/xs:attribute[not(@msdata:AutoIncrement='true') and not(@msdata:ReadOnly='true')]">
          ...
          if( <xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property"/>() != null <xsl:if test="@type = 'xs:string'"><![CDATA[&&]]> !<xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property"/>().equals("")</xsl:if> <xsl:if test="codegen:IsBinary( @type)"><![CDATA[&&]]> <xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property"/>().length != 0 </xsl:if> )
              ...
              <xsl:choose>
                <xsl:when test="@type = 'xs:date' or @type = 'xs:oracleTimestamp'">
      ...
      <xsl:variable name="fk" select="@name" />
        <xsl:for-each select="key('table',$child-table-pk)[@wdm:Schema = $child-table-pk-schema]">
              ...
              <![CDATA[&&]]> <xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property-name" />Items().size() &gt; 0)
          {
            ...
            {
              <xsl:value-of select="codegen:getServerNamespace(@wdm:Schema)"/>.<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/> item = (<xsl:value-of select="codegen:getServerNamespace(@wdm:Schema)"/>.<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)"/>)<xsl:value-of select="$functionParam" />.get<xsl:value-of select="$property-name" />Items().get( i );
          ...
          resultSet.next();
          <xsl:value-of select="$functionParam" />.set<xsl:value-of select="codegen:getPropertyName( $table,$schema,xs:complexType/xs:attribute[@msdata:AutoIncrement='true']/@name)"/>( <xsl:value-of select="codegen:getParseFunction( xs:complexType/xs:attribute[@msdata:AutoIncrement='true']/@type)" />( Double.parseDouble( resultSet.getString( 1 ) ) ) ) ;
          resultSet.close();
            </xsl:when>
            <xsl:otherwise>
          sqlCommand.execute();

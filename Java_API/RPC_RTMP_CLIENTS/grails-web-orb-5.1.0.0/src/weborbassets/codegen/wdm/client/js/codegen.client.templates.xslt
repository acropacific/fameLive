...
var webOrbURL = "<xsl:value-of select="@serverRootURL" />/weborb.wo?clientid=" + __clientId__;
var webOrbMessagingURL = "<xsl:choose>
        <xsl:when test="@webSocketServerURL"><xsl:value-of select="@webSocketServerURL"/></xsl:when>
        <xsl:otherwise><xsl:value-of select="@serverRootURL"/></xsl:otherwise>
      </xsl:choose>";

webORB.defaultMessagingURL = webOrbMessagingURL;

webORB.setDefaultRemotingURL( "<xsl:value-of select="@serverRootURL" />/weborb.wo" );
</xsl:if>
    </xsl:for-each>
...
<!-- ==========================  CODEGEN.CLIENT.MODEL.EXT -->
  ...
  <xsl:template name="codegen.client.database.codegen">
    ...
    <xsl:variable name="class-name" select="codegen:getClassNameJS($table,$schema)" />
    <xsl:variable name="class-name-db" select="codegen:getClassNameJSPrefix($table,$schema,'_')" />
    <xsl:variable name="class-name-server" select="codegen:getClassName($table,$schema)" />
...
<xsl:value-of select="$class-name-db"/>Db.prototype.<xsl:value-of select="codegen:getStoredProcedureName($storedProcedure, $storedProcedureSchema)"/> = function( <xsl:for-each select="xs:complexType/xs:attribute"><xsl:value-of select="codegen:getStoredProcedureParameter($storedProcedure,$storedProcedureSchema,@name)" /><xsl:if test="position() != last()">, </xsl:if></xsl:for-each> )
{
    var remoteObject = this._createRemoteObject();
    var async = new Async();
    remoteObject.<xsl:value-of select="codegen:getStoredProcedureName($storedProcedure, $storedProcedureSchema)"/>( <xsl:for-each select="xs:complexType/xs:attribute"><xsl:value-of select="codegen:getStoredProcedureParameter($storedProcedure,$storedProcedureSchema,@name)" />, </xsl:for-each>async );
    return async;
}
</xsl:for-each>
...
<xsl:for-each select="xs:complexType/xs:choice/xs:element[@wdm:DatabaseObjectType='table']">
  ...
  <xsl:variable name="schema" select="@wdm:Schema" />
  <xsl:variable name="class-name" select="codegen:getClassNameJS($table,$schema)" />
  <xsl:variable name="class-name-getter" select="codegen:getClassNameForField($table,$schema)" />

<xsl:value-of select="$schema"/>._DataMapperRegistry.prototype.get<xsl:value-of select="$class-name-getter"/> = function()
{
    if( this._<xsl:value-of select="codegen:getFunctionParameter($table)"/>DataMapper == null )
        this._<xsl:value-of select="codegen:getFunctionParameter($table)"/>DataMapper = new <xsl:value-of select="$class-name"/>DataMapper();

    return this._<xsl:value-of select="codegen:getFunctionParameter($table)"/>DataMapper;
}
</xsl:for-each>
//	endof _DataMapperRegistry class
  ...
  <xsl:template name="codegen.client.datamapper.codegen">
	...
	<xsl:variable name="schema" select="@wdm:Schema"   />
    <xsl:variable name="class-name" select="codegen:getClassNameJS($table,$schema)" />
    <xsl:variable name="class-name-prefix" select="codegen:getClassNameJSPrefix($table,$schema,'_')" />
    <xsl:variable name="class-name-server" select="codegen:getClassName($table,$schema)" />
...
<xsl:value-of select="$class-name-prefix"/>DataMapper.prototype.load = function( <xsl:value-of select="codegen:getFunctionParameter($class-name)" />, async )
{
    ...
    if( async == null )
    {
        <xsl:value-of select="codegen:getFunctionParameter($class-name)" /> = this._createRemoteObject().findByPrimaryKey( <xsl:for-each select="xs:key/xs:field"><xsl:value-of select="codegen:getFunctionParameter($class-name)" />.<xsl:value-of select="codegen:getPropertyName($table, $schema, substring(@xpath,2),1)"/><xsl:if test="position() != last()">, </xsl:if></xsl:for-each> );
...
<xsl:value-of select="$class-name-prefix"/>DataMapper.prototype.findByPrimaryKey = function( <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]"><xsl:value-of select="codegen:getFunctionParameter(@name)" /><xsl:if test="position() != last()">, </xsl:if></xsl:for-each> )
{
    ...
    <xsl:for-each select="xs:key/xs:field">activeRecord.<xsl:value-of select="codegen:getPropertyName($table, $schema, substring(@xpath,2),1)"/> = <xsl:value-of select="codegen:getFunctionParameter(substring(@xpath,2))"/>;
    </xsl:for-each>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <xsl:variable name="dynamic-function">findBy( "<xsl:for-each select="key('fkByName',$fk)">
              ...
              <xsl:if test="position() != 1">, </xsl:if>activeRecord.<xsl:value-of select="codegen:getPropertyName(../../@name, ../../@wdm:Schema, substring(@xpath,2),1)"/>
...
<xsl:value-of select="$class-name-prefix"/> = function( <!--xsl:for-each select="xs:complexType/xs:attribute[not(concat('@',@name) = key('fk',$class-name)/@xpath)]"--><xsl:for-each select="xs:complexType/xs:attribute"><xsl:value-of select="codegen:getFunctionParameter(codegen:getPropertyName($table,$schema,@name,1))" /><xsl:if test="position() != last()">, </xsl:if></xsl:for-each> )
{
...
<xsl:for-each select="xs:complexType/xs:attribute">
    ...
    this._columnsInfo = 
    [<xsl:for-each select="xs:complexType/xs:attribute">
      new ColumnInfo( "<xsl:value-of select="codegen:getPropertyName($table,$schema,@name,1)"/>", "<xsl:value-of select="codegen:getJSDataType(@type)" />", <xsl:choose><xsl:when test="../../xs:key/xs:field[@xpath = concat('@',current()/@name)]">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>, <xsl:choose><xsl:when test="@use = 'required'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose>, <xsl:choose><xsl:when test="@msdata:AutoIncrement = 'true'">true</xsl:when><xsl:otherwise>false</xsl:otherwise></xsl:choose> )<xsl:if test="position() != last()">, </xsl:if>
    </xsl:for-each> 
    ];
    <xsl:for-each select="key('dependent',current()/xs:key/@name)">
    <xsl:variable name="child-table" select="@name" />
...
<xsl:value-of select="$class-name-prefix"/>.prototype.get<xsl:value-of select="codegen:getParentProperty($table, $schema, $parent-table, $parent-schema, $key,0)" /> = function()
{
    ...
    <xsl:for-each select="xs:filed">
      <xsl:value-of select="$var-name-s" />.<xsl:value-of select="codegen:getPropertyName($parent-table,$parent-schema,substring(@related,2),1) "/> =  = this.<xsl:value-of select="codegen:getPropertyName($table,$schema,substring(@xpath,2),1) "/>;
...
<xsl:value-of select="$class-name-prefix"/>.prototype.getURI = function()
{
    return "<xsl:value-of select="/xs:schema/runtime/@modelName"/>.<xsl:value-of select="../../../@name"/>.<xsl:value-of select="$schema"/>.<xsl:value-of select="@name"/>"
        <xsl:for-each select="xs:key/xs:field">+ "." + this.<xsl:value-of select="codegen:getPropertyName($table, $schema, substring(@xpath,2),1)" />.toString()</xsl:for-each>;
}
...
<!--    
    ...
    refForRelated.ActiveRecordUID = activeRecord.ActiveRecordUID;<xsl:for-each select="xs:complexType/xs:attribute">
      <xsl:variable name="pkName" select="codegen:getPropertyName($table,$schema,@name,1)" />
...
{
	return "<xsl:value-of select="codegen:getServerNamespace(@wdm:Schema)" />.<xsl:value-of select="$class-name-server"/>";
}
//	endof <xsl:value-of select="$class-name-prefix"/> class
...
webORB.registerTypeFactory( "<xsl:value-of select="codegen:getServerNamespace(@wdm:Schema)" />.<xsl:value-of select="$class-name-server"/>", function(object)
{

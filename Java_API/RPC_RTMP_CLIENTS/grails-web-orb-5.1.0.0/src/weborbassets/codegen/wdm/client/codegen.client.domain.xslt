...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template name="codegen.client.domain.codegen">
    ...
    <file name="{concat('_',codegen:getClassName(@name,@wdm:Schema))}.as" override="true" >
      ...
      import <xsl:value-of select="codegen:getClientNamespace(current()/@item, 0)" />.*;
      </xsl:for-each>
      ...
      <xsl:for-each select="xs:keyref">
        <xsl:variable name="refer-schema" select="@refer-schema" />
        // parent tables
        internal var _<xsl:value-of select="codegen:getParentProperty($table, $schema, key('table',@refer)[@wdm:Schema = $refer-schema]/@name, $refer-schema, @name,1)" />:<xsl:text> </xsl:text><xsl:value-of select="codegen:getClassNameClient(key('table',@refer)[@wdm:Schema = $refer-schema]/@name,$refer-schema)"/>
        <!--<xsl:if test="xs:field[substring(@xpath,2) = ../../xs:complexType/xs:attribute[@use='required']/@name]">-->
            <!--= new <xsl:value-of select="codegen:getClassNameClient(key('table',@refer)[@wdm:Schema = $refer-schema]/@name,$refer-schema)"/>()-->
        <!--</xsl:if>-->;
      </xsl:for-each>
      ...
      <xsl:for-each select="xs:complexType/xs:attribute">
        ...
        <xsl:variable name="property" select="codegen:getPropertyName($table, $schema, @name, 1)" />
          ...
          <xsl:otherwise>
            ...
            <xsl:for-each select="key('tableById',$id)/xs:keyref">
              <xsl:for-each select="xs:field">
                ...
                <xsl:if test="@xpath = concat('@',$attribute-name)">
                  ...
                  return <xsl:value-of select="$parent-property"/>.<xsl:value-of select="codegen:getPropertyName($parent-table-name, $parent-table-schema, substring($parent-pk-field-name,2),1) "/>;
			...
			<xsl:for-each select="key('tableById',$id)/xs:keyref">
              <xsl:for-each select="xs:field">
                ...
                <xsl:if test="@xpath = concat('@',$attribute-name)">
                  <!--xsl:variable name="pk-field-position" select="position()" /-->
                    <xsl:variable name="parent-table-schema" select="../@refer-schema" />
                    <xsl:variable name="parent-table-name" select="key('table', ../@refer)[@wdm:Schema = $parent-table-schema]/@name" />
                  ...
                  <xsl:value-of select="$parent-property"/>.<xsl:value-of select="codegen:getPropertyName($parent-table-name, $parent-table-schema, substring($parent-pk-field-name,2),1)"/> = value;
                </xsl:if>
              </xsl:for-each>
            </xsl:for-each>
      ...
      <xsl:for-each select="xs:keyref">
        ...
        public function get <xsl:value-of select="codegen:getParentProperty($table, $schema, $parent-table, $parent-schema, $key,1)" />():<xsl:value-of select="$parent-class"/>
        {
        ...
        public function set <xsl:value-of select="codegen:getParentProperty($table, $schema, $parent-table, $parent-schema, $key,1)" />(value:<xsl:value-of select="codegen:getClassNameClient($parent-table,$parent-schema)"/>):void
        {
          if( value != null )
          {
          	var oldValue:ActiveRecord = <xsl:value-of select="$var-name" />;
            ...
            <xsl:variable name="refer-schema" select="@refer-schema" />
            <xsl:value-of select="$var-name" /> = <xsl:value-of select="codegen:getClassNameClient(key('table',@refer)[@wdm:Schema = $refer-schema]/@name,$refer-schema)" />(IdentityMap.global.register( value ));
      ...
      <xsl:for-each select="key('dependent',current()/xs:key/@name)">
        ...
        <xsl:for-each select="xs:keyref[@refer = $pk and @refer-schema = $schema]">
		  ...
		  <xsl:if test="$child-schema = @wdm:Schema">
            <xsl:choose>
              <xsl:when test="count(xs:key/xs:field[@xpath = key('fkByName',$fk)/@xpath]) = count(xs:key/xs:field)">
                ...
                [Bindable(event="loaded")]
                public function get <xsl:value-of select="codegen:getChildProperty($table,$schema,$child-table,$child-schema,$fk,1)" />():<xsl:value-of select="codegen:getClassNameClient(@name,@wdm:Schema)"/>
                {
                ...
                <xsl:for-each select="key('pkByName',$pk)[../@wdm:Schema = $schema]">
						<xsl:if test="position() != 1">, </xsl:if>
							<xsl:value-of select="codegen:getPropertyName($table, $schema, substring(@xpath,2),1)"/>
                </xsl:for-each>);
                ...
                public function set <xsl:value-of select="codegen:getChildProperty($table,$schema,$child-table,$child-schema,$fk,1)" />(value:<xsl:value-of select="codegen:getClassNameClient(@name,@wdm:Schema)" />):void
                {
              ...
              <xsl:otherwise>
                ...
                [Bindable(event="loaded")]
                public function get <xsl:value-of select="codegen:getChildProperty($table,$schema,$child-table,$child-schema,$fk,1)" />():ActiveCollection
                {
                  <xsl:value-of select="$var-name" /> = onChildRelationRequest("<xsl:value-of select="$hidden-property" />Items",<xsl:value-of select="$var-name" />);
      ...
      <xsl:if test="xs:keyref">
        protected override function onDirtyChanged():void
        {
          <xsl:for-each select="xs:keyref">
			...
			<xsl:variable name="parent-schema" select="key('table',@refer)/@refer-schema" />
      ...
      <xsl:for-each select="xs:keyref">
        <xsl:variable name="refer-schema" select="@refer-schema" />
        <xsl:variable name="property" select="codegen:getParentProperty($table, $schema, key('table',@refer)[@wdm:Schema = $refer-schema]/@name, $refer-schema, @name,1)" />
      ...
      if( this._<xsl:value-of select="$property"/> != null &amp;&amp; this._<xsl:value-of select="$property"/>.IsPrimaryKeyInitialized )
      {
        ...
        this._<xsl:value-of select="codegen:getParentProperty($table, $schema, key('table',@refer)[@wdm:Schema = $refer-schema]/@name, $refer-schema, @name,1)" />.prepareToSend(identityMap,false) as <xsl:value-of select="codegen:getClassNameClient(key('table',@refer)[@wdm:Schema = $refer-schema]/@name, $refer-schema)"/>;
      }
    </xsl:for-each>
              ...
              <xsl:for-each select="key('table',$child-table-pk)">
			   <xsl:if test="$child-schema = @wdm:Schema">
                <xsl:choose>
                  ...
                  <xsl:otherwise>
                    <xsl:variable name="hidden-property" select="codegen:getChildProperty($table,$schema,@name,@wdm:Schema,$fk,1)" />
                    if(this["<xsl:value-of select="$hidden-property"/>Items"])
                    {
                      for each(var <xsl:value-of select="$child-var"/> :ActiveRecord in this["<xsl:value-of select="$hidden-property"/>Items"] as Array)
      ...
      public override function applyFields(object:Object):void
      {
        ...
        <xsl:for-each select="xs:complexType/xs:attribute">
          ...
          <xsl:for-each select="xs:keyref">
            <xsl:variable name="parent-table" select="key('table',@refer)/@name" />
			<xsl:variable name="parent-schema" select="key('table',@refer)/@refer-schema" />
            <xsl:value-of select="codegen:getParentProperty($table,$schema,$parent-table,$parent-schema,@name,1)" /> =
            object.<xsl:value-of select="codegen:getParentProperty($table,$schema,$parent-table,$parent-schema, @name,1)" />;
          </xsl:for-each>
        ...
        public override function getURI():String
        {
          ...
          if(_uri == null)
          {
           _uri = "<xsl:value-of select="codegen:getClientNamespace( $schema, true() )"/>.<xsl:value-of select="@name"/>"
            <xsl:for-each select="xs:key/xs:field">
              + "." + <xsl:value-of select="codegen:getPropertyName($table, $schema, substring(@xpath,2),1)" />.toString()
            </xsl:for-each>;
          }
    ...
    <file name="{concat('_',$class-name,'DataMapper')}.as" override="true">
      package <xsl:value-of select="codegen:getClientNamespace(codegen:getCurrentSchema(), 1)" />
      {
        ...
        <xsl:for-each select="codegen:GetSchemas(codegen:getCurrentDatabase())">
        import <xsl:value-of select="codegen:getClientNamespace(current()/@item, 0)" />.DataMapperRegistry;
        </xsl:for-each>
        ...
        public dynamic class _<xsl:value-of select="$class-name"/>DataMapper extends DataMapper
        {
      		...
      		public function load(<xsl:value-of select="codegen:getFunctionParameter($class-name)" />:<xsl:value-of select="$class-name"/>, responder:Responder = null):<xsl:value-of select="$class-name"/>
          {
              ...
              var asyncToken:AsyncToken = new DatabaseAsyncToken(createRemoteObject().findByPrimaryKey(
                <xsl:for-each select="xs:key/xs:field">
                  <xsl:value-of select="codegen:getFunctionParameter($class-name)" />.<xsl:value-of select="codegen:getPropertyName($table, $schema, substring(@xpath,2),1)"/>
          ...
          -->
            ...
            <xsl:for-each select="xs:key/xs:field">
              activeRecord.<xsl:value-of select="codegen:getPropertyName($table, $schema, substring(@xpath,2),1)"/> = <xsl:value-of select="codegen:getFunctionParameter(substring(@xpath,2))"/>;
            </xsl:for-each>
        ...
        public function createInstanceByPrimaryKey( <xsl:for-each select="xs:complexType/xs:attribute[concat('@',@name) = key('pk',$table)/@xpath]">
        ...
        </xsl:for-each>):<xsl:value-of select="$class-name" />
          {
            ...
            <xsl:for-each select="xs:key/xs:field">
              activeRecord.<xsl:value-of select="codegen:getPropertyName($table,substring(@xpath,2),1)"/> = <xsl:value-of select="codegen:getFunctionParameter(substring(@xpath,2))"/>;
            </xsl:for-each>
              ...
              <xsl:otherwise>
                ...
                <xsl:variable name="dynamic-function">
                  ...
                  <xsl:for-each select="key('pkByElementId',$id)">
                    <xsl:if test="position() != 1">,</xsl:if>
                    item.<xsl:value-of select="codegen:getPropertyName(../../@name, ../../@wdm:Schema, substring(@xpath,2),1)"/>
                  </xsl:for-each>, activeCollection,getRelationQueryOptions(relationName))
                </xsl:variable>

                if(relationName == "<xsl:value-of select="$hidden-property" />Items")
                {
                <!--xsl:choose>
      ...
      <xsl:for-each select="xs:complexType/xs:choice/xs:element[@wdm:DatabaseObjectType='table']">
       ...
       public class _DataMapperRegistry
       {
        <xsl:for-each select="xs:complexType/xs:choice/xs:element[@wdm:DatabaseObjectType='table']">
          ...
          public function get <xsl:text> </xsl:text><xsl:value-of select="$class-name"/>():<xsl:value-of select="$class-name"/>DataMapper
          {
    ...
    <file name="_{codegen:getClassName(@name,@wdm:Schema)}Db.as">
      package <xsl:value-of select="codegen:getClientNamespace(codegen:getCurrentSchema(), 1)" />
      {
        ...
        public class _<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />Db extends Database
        {
          ...
          protected override function get RemoteClassName():String
          {
            return "<xsl:value-of select="codegen:getServerNamespace(codegen:getCurrentSchema())" />.<xsl:value-of select="codegen:getClassName(@name,@wdm:Schema)" />Db";
          }

...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="codegen.datatype">
    <xsl:for-each select="datatype">
      function <xsl:call-template name='codegen.datatype.name'/>()
      {
      }

      <xsl:call-template name='codegen.datatype.name'/>.prototype =
      {
      <xsl:call-template name="codegen.datatype.loadfields"/>
      }

      function <xsl:call-template name='codegen.datatype.name'/>Factory( obj )
      {
      var new<xsl:value-of select="@name" />Obj = new <xsl:call-template name='codegen.datatype.name'/>();
  ...
  <xsl:template name="codegen.datatype.field">
    ...
    <xsl:when test="@type='Array'">
      ...
      <xsl:with-param name="datatype-name"><xsl:call-template name="codegen.datatype.name"><xsl:with-param name="name" select="@elementType"/></xsl:call-template></xsl:with-param>
    </xsl:call-template>]
    </xsl:when>
    <xsl:when test="@type='Object'">null</xsl:when>
    <xsl:otherwise>
      <xsl:call-template name="codegen.get.typeprocessor">
        <xsl:with-param name="datatype-name"><xsl:call-template name="codegen.datatype.name"><xsl:with-param name="name" select="@type"/></xsl:call-template></xsl:with-param>
      </xsl:call-template>
    </xsl:otherwise>
  </xsl:choose><xsl:text>,
      </xsl:text>
   ...
   <xsl:variable name="nameWithSlashes">
    <xsl:call-template name="string-replace-all">
      ...
      <xsl:with-param name="by" select="'/'" />
		...
		<file name="{@name}Invoker.html"><![CDATA[<html><head>
...
var serviceClass = "]]><xsl:call-template name="codegen.datatype.name"/><![CDATA[";
var service = new ]]><xsl:call-template name="codegen.datatype.name"/><![CDATA[();
var methodInfo = []]><xsl:for-each select="method">
        {name:'<xsl:call-template name="codegen.datatype.name"/>', args:[ <xsl:for-each select="arg"><xsl:if test="position() != 1">, </xsl:if>
<xsl:call-template name="codegen.get.typeprocessor"><xsl:with-param name="datatype-name"><xsl:call-template
        name="codegen.datatype.name"><xsl:with-param name="name" select="@type"/></xsl:call-template></xsl:with-param></xsl:call-template>
...
initDisplay();
  ...
  <xsl:template name="codegen.get.typeprocessor">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:text>{__woproto__:"</xsl:text><xsl:value-of select='$datatype-name' /><xsl:text>"}</xsl:text>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template name='codegen.datatype.name'>
...
<xsl:template name="codegen.code">
	...
	{
		this.proxy = webORB.bind( "<xsl:value-of select='@fullname' />", "<xsl:value-of select='@endpointURL' />" );
  ...
  <xsl:template name="codegen.datatype.registry">
    <file name="DatatypeRegistry.js">
      ...
      <xsl:if test='count(//datatype)>0'>
        <xsl:for-each select='//datatype'>
          webORB.registerTypeFactory( "<xsl:value-of select='@typeNamespace'/>.<xsl:value-of select='@name'/>", <xsl:call-template name='codegen.datatype.name'/>Factory );
        </xsl:for-each>
      </xsl:if>
      }
    </file>

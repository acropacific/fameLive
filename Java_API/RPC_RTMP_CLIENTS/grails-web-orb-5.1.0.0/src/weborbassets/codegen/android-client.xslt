<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="codegen.process.fullproject">
        <folder name="Intellij Idea 10">
            <folder name="InvokerDemoApp">
                ...
                <folder name="src">
                    <xsl:variable name="views_pkg_exists" select="/namespaces/namespace/@fullname = 'com'"/>
                    <xsl:variable name="models_pkg_exists" select="/namespaces/namespace/@fullname = 'models'"/>
                    <xsl:variable name="controllers_pkg_exists" select="/namespaces/namespace/@fullname = 'controllers'"/>

                    <xsl:if test="not($views_pkg_exists)">
                        <folder name="com">
                            <folder name="example">
                                <xsl:call-template name="codegen.project.src.views">
                    ...
                    <xsl:if test="not($models_pkg_exists)">
                        <folder name="models">
                            <xsl:call-template name="codegen.project.src.models">
                                <xsl:with-param name="src_files_path" select="concat($intellij_idea_files_path, 'src/')"/>
                            </xsl:call-template>
                        </folder>
                    </xsl:if>

                    <xsl:if test="not($controllers_pkg_exists)">
                        <folder name="controllers">
                            <xsl:call-template name="codegen.project.src.controllers">
                                <xsl:with-param name="src_files_path" select="concat($intellij_idea_files_path, 'src/')"/>
                            </xsl:call-template>
                        </folder>
                    </xsl:if>
                    ...
                    <xsl:for-each select="/namespaces">
                        <xsl:call-template name="codegen.process.namespace.project.src">
                            <xsl:with-param name="do_views" select="$views_pkg_exists"/>
                            <xsl:with-param name="do_models" select="$models_pkg_exists"/>
                            <xsl:with-param name="do_controllers" select="$controllers_pkg_exists"/>
                            <xsl:with-param name="src_files_path" select="concat($intellij_idea_files_path, 'src/')"/>
                        </xsl:call-template>
                    </xsl:for-each>
                </folder>
    ...
    <xsl:template name="codegen.process.namespace.project.src">
        <xsl:param name="do_views"/>
        <xsl:param name="do_models"/>
        <xsl:param name="do_controllers"/>
        <xsl:param name="src_files_path"/>

        <xsl:for-each select="namespace">
            <folder name="{@name}">
                <xsl:call-template name="codegen.process.namespace.project.src">
                    <xsl:with-param name="do_views" select="$do_views"/>
                    <xsl:with-param name="do_controllers" select="$do_controllers"/>
                    <xsl:with-param name="do_models" select="$do_models"/>
                    <xsl:with-param name="src_files_path" select="$src_files_path"/>
                </xsl:call-template>

                <xsl:choose>
                    <xsl:when test="$do_views and @fullname = 'com.example'">
                        <xsl:call-template name="codegen.project.src.views">
                            <xsl:with-param name="src_files_path" select="$src_files_path"/>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:when test="$do_views and @fullname = 'com' and not(namespace/@fullname = 'com.example')">
                        <folder name="example">
                            <xsl:call-template name="codegen.project.src.views">
                                <xsl:with-param name="src_files_path" select="$src_files_path"/>
                            </xsl:call-template>
                    ...
                    <xsl:when test="$do_models and @fullname = 'models'">
                        <xsl:call-template name="codegen.project.src.models">
                            <xsl:with-param name="src_files_path" select="$src_files_path"/>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:when test="$do_controllers and @fullname = 'controllers'">
                        <xsl:call-template name="codegen.project.src.controllers">
                            <xsl:with-param name="src_files_path" select="$src_files_path"/>
                        </xsl:call-template>
                    </xsl:when>
...
public class </xsl:text><xsl:value-of select="@name"/><xsl:value-of select="@genericParameters"/>
<xsl:text>
{</xsl:text>
...
public </xsl:text><xsl:value-of select="name()"/><xsl:text> </xsl:text><xsl:value-of select="@name"/><xsl:text> {
    ...
    <xsl:template name="codegen.project.intellij_idea">
        ...
        <folder name="assets"/>
        <folder name="bin"/>
        <!-- 'gen' folder is auto generated -->
        <folder name="libs">
            <file path="../javaclient/weborbclient.jar" hideContent="true"/>
        </folder>
        ...
        <folder name=".idea">
            ...
            <folder name="libraries">
                <file path="{concat($app_files_path, '.idea/libraries/weborbclient_jar.xml')}"/>
            </folder>
    ...
    <xsl:template name="codegen.project.src.views">
        <xsl:param name="src_files_path"/>
    ...
    <xsl:template name="codegen.project.src.controllers">
        <xsl:param name="src_files_path"/>
    ...
    <xsl:template name="codegen.project.src.models">
        <xsl:param name="src_files_path"/>
...
public class AppModel {
    ...
    private void loadProperties() {
        ...
        if (WebORBURL == null) {
            WebORBURL = "</xsl:text>
            <xsl:variable name="url" select="//service/@endpointURL"/>
            <xsl:choose>

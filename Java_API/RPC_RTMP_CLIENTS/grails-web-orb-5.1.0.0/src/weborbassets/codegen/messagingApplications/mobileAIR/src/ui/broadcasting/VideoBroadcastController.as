package ui.broadcasting
{
	...
	public class VideoBroadcastController
	{
		...
		{
			_view.videoContainerParent.addElement( new BroadcastViewer() );
		}
		...
		private function startBroadcast():void
		{
			...
			_camera = Camera.getCamera( model.selectedCameraIndex.toString() );
			...
			if (_camera)
			{
				...
				if( _view.currentState == "landscapeSmall" )
					videoContainer.height = 120;
				else if( _view.currentState == "landscapeLarge" )
					videoContainer.height = 240;
				else if( _view.currentState == "portraitLarge" )
					videoContainer.height = 240;
				else if( _view.currentState == "portraitSmall" )
					videoContainer.height = 120;
				
				_video.width = _view.videoContainerParent.width;
				_video.height = videoContainer.height;
				
				videoContainer.addChild(_video);
				_view.videoContainerParent.addElement( videoContainer );
			}
		...
		private function stopBroadcast():void
		{
			if (_video)
			{
				...
				_view.videoContainerParent.removeAllElements();
				_video = null;
			}
		...
		private function onNetStatus(event:NetStatusEvent):void
		{
			switch (event.info.code)
			{
				...
				case "NetConnection.Connect.Failed":
					 new Alert().show("The connection attempt failed.", _view);
					break;
				case "NetStream.Publish.BadName":
					new Alert().show("Attempt to publish a stream which is already being published by someone else.", _view);
					break;
				case "NetStream.Publish.Start":
					_model.broadcasting = true;
					break;
				case "NetStream.Connect.Failed":
					new Alert().show("The P2P connection attempt failed.", _view);
					break;
				case "NetConnection.Connect.Closed":
					_model.connected = false;
					// don't break here
		...
		{
			new Alert().show( "Received async error - " + evt.error.toString(), _view );
		}

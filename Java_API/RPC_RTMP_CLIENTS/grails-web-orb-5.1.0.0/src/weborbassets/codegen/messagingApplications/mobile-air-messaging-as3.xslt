...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:codegen="urn:cogegen-xslt-lib:xslt">
  <xsl:template match="/">
    <folder name="weborb-codegen">
      <info>info text</info>
      <folder name="src">
        <file path="mobileAIR/src/MessagingCodegenAndroid.mxml" />
        <file path="mobileAIR/src/MessagingCodegenAndroid-app.xml" />
        <folder name="ui">
          <xsl:for-each select="data/features/feature">
            <xsl:choose>
              <xsl:when test="id = 0">
                <folder path="mobileAIR/src/ui/broadcasting" />
              </xsl:when>
              <xsl:when test="id = 1">
                <folder path="mobileAIR/src/ui/recording" />
              </xsl:when>
              <xsl:when test="id = 2">
                <folder path="mobileAIR/src/ui/sharedObj" />
              </xsl:when>
              <xsl:when test="id = 3">
                <folder path="mobileAIR/src/ui/dataPush" />
              </xsl:when>
              <xsl:when test="id = 4">
                <folder path="mobileAIR/src/ui/methodCall" />
              </xsl:when>
            </xsl:choose>
          </xsl:for-each>
          <folder path="mobileAIR/src/ui/controls" />
          <folder path="mobileAIR/src/ui/enums" />
          <folder path="mobileAIR/src/ui/events" />
          <folder name="home">
            <file path="mobileAIR/src/ui/home/HomeController.as" />
            <file name="HomeView.mxml"><![CDATA[<?xml version="1.0" encoding="utf-8"?>
        ...
        <![CDATA[
            ...
            {]]>
              <xsl:choose>
                ...
                {]]>
                <xsl:for-each select="data/features/feature">
                  <xsl:if test="id != 5"><![CDATA[
                    case ]]><xsl:value-of select="position() - 1"/><![CDATA[:]]> 
                  </xsl:if>
                  <xsl:choose>
                    <xsl:when test="id = 0">
                        navigator.pushView(VideoBroadcastView);
    ...
    <s:Label id="lblCurrentUri" width="100%" color="#838383" 
        text="{ConnectionManager.instance.connectionUri}" />
          ...
          <folder name="managers">
            <file name="ConnectionManager.as"><![CDATA[package ui.managers
{
    ...
    public class ConnectionManager extends EventDispatcher
    {
        ...
        {]]>
            ...
            return "rtmpt://localhost:<xsl:value-of select="/data/weborbHTTPPort"/>" + "/<xsl:value-of select="/data/applicationName"/>";
                </xsl:when>
                </xsl:choose>
              </xsl:for-each><![CDATA[
        }
...
class PrivateConnectionManager
{
          ...
          <file path="mobileAIR/src/ui/Defaults.as" />
        </folder>
        <folder path="mobileAIR/src/assets" />
      </folder>
      <folder name="libs">
...
<actionScriptProperties analytics="false" mainApplicationPath="MessagingCodegenAndroid.mxml" projectUUID="54277c98-2a1b-4e6f-b737-dcd9565c11a9" version="10">
  ...
  <buildTargets>
    ...
    <buildTarget buildTargetName="com.adobe.flexide.multiplatform.ios.platform" iosSettingsVersion="1" provisioningFile="" releasePackageType="">
      ...
      <airSettings airCertificatePath="" airTimestamp="true" version="1">
        <airExcludes>
          <pathEntry path="blackberry-tablet.xml"/>
        </airExcludes>
      </airSettings>
      <actionScriptSettings version="1"/>
    </buildTarget>
    <buildTarget buildTargetName="com.qnx.flexide.multiplatform.qnx.platform" extraPackagingOptions="" signBarFile="false">
      ...
      <file path="mobileAIR/.flexProperties" />

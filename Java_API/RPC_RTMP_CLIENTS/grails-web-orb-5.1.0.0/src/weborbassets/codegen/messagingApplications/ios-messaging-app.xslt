  ...
  <xsl:template match="/">
    <folder name="weborb-codegen">      
        <folder name="MessagingApps">
          <folder path="ios/MessagingApps.xcodeproj" />  
          <folder path="ios/art" />  
          <folder name="MessagingApps">
            <folder path="ios/MessagingApps/en.lproj" />
            <file path="ios/MessagingApps/MessagingApps-Info.plist" />
            <file path="ios/MessagingApps/MessagingApps-Prefix.pch" />
            <file path="ios/MessagingApps/main.m" />
            <xsl:for-each select="data/features/feature">
              <xsl:choose>
                <xsl:when test="id = 0">
                  <folder path="ios/MessagingApps/VideoBroadcast" />
                </xsl:when>
                <xsl:when test="id = 1">
                  <folder path="ios/MessagingApps/VideoPLayer" />
                </xsl:when>
                <xsl:when test="id = 2">
                  <folder path="ios/MessagingApps/RemoteSO" />
                  <folder path="ios/MessagingApps/PhotoRSO" />
                </xsl:when>
                <xsl:when test="id = 3">
                  <folder path="ios/MessagingApps/DataPush" />
                </xsl:when>
                <xsl:when test="id = 4">
                  <folder path="ios/MessagingApps/MethodInvocation" />
                </xsl:when>
              </xsl:choose>
            </xsl:for-each>           
            <file path="ios/MessagingApps/AppDelegate.h" />
...
+(NSString *)defaultHostURL {
    ...
    if ([scheme isEqualToString:@"rtmp"] || [scheme isEqualToString:@"rtmps"])
        ...
        return [NSString stringWithFormat:@"rtmp://%@:1935", [url host]];
}

...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:codegen="urn:cogegen-xslt-lib:xslt">
    <xsl:template match="/">
      <folder name="weborb-codegen">
        <info>info text</info>
        <folder name="src">
          <file name="RTMPSampleApp.mxml"><![CDATA[<?xml version="1.0" encoding="utf-8"?>
  ...
  <xsl:choose>
    ...
    <xsl:when test="id = 4"><![CDATA[
        <mx:Canvas label="Server Method Invocation" width="100%" height="100%">
          ...
          <file path="air/src/RTMPSampleApp-app.xml" />
          <folder name="ui">
            <xsl:for-each select="data/features/feature">
              <xsl:choose>
                <xsl:when test="id = 0">
                  <folder path="air/src/ui/broadcasting" />
                </xsl:when>
                <xsl:when test="id = 1">
                  <folder path="air/src/ui/recording" />
                </xsl:when>
                <xsl:when test="id = 2">
                  <folder path="air/src/ui/sharedObj" />
                </xsl:when>
                <xsl:when test="id = 3">
                  <folder path="air/src/ui/dataPush" />
                </xsl:when>
                <xsl:when test="id = 4">
                  <folder path="air/src/ui/methodCall" />
                </xsl:when>
              </xsl:choose>
            </xsl:for-each>
            <file path="air/src/ui/Defaults.as" />
...
package ui.managers
{
    ...
    public class ServiceManager
    {
        ...
        {]]>
            ...
            var uri:String = "rtmpt://localhost:<xsl:value-of select="/data/weborbHTTPPort"/>";
              </xsl:when>

...
<projectDescription>
	...
	<natures>
        ...
        <folder path="flex/.settings"/>
        <folder path="flex/html-template"/>
				<folder name="src">
    ...
    <xsl:for-each select="data/features/feature">
      ...
      <xsl:when test="id = 4">
        <![CDATA[<mx:Canvas label="Server Method Invocation" width="100%" height="100%">
          ...
          <folder name="ui">
            <file path="flex/src/ui/Defaults.as"/>
            <xsl:for-each select="data/features/feature">
              <xsl:choose>
                <xsl:when test="id = 0">
                  <folder path="flex/src/ui/broadcasting"/>
                </xsl:when>
                <xsl:when test="id = 1">
                  <folder path="flex/src/ui/recording"/>
                </xsl:when>
                <xsl:when test="id = 2">
                  <folder path="flex/src/ui/sharedObj"/>
                </xsl:when>
                <xsl:when test="id = 3">
                  <folder path="flex/src/ui/dataPush"/>
                </xsl:when>
                <xsl:when test="id = 4">
                  <folder path="flex/src/ui/methodCall"/>
                </xsl:when>
              </xsl:choose>
            </xsl:for-each>           
